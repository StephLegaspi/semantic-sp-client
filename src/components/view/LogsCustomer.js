...
class Logs extends Component {
	...
	searchByID = () => {
        let self = this;
        fetch('http://localhost:3001/v1/logs/customer/' + self.state.log_id, {
            method: 'GET'
	...
	componentDidMount() {
        let self = this;
        fetch('http://localhost:3001/v1/logs/customer', {
            method: 'GET'
        }).then(function(response) {
            if (response.status >= 400) {
                throw new Error("Bad response from server");
	...
	render() {
		return (
			<div>
				<HeaderBar headerTitle={'Customer Logs'}/>
				<SearchBarTable titleHolder={'Search customer ID..'} searchData={this.searchByID} inputChange={this.handleIDChange}/>
				...
				<Table>
				    <Table.Header>
				      <Table.Row>
				        ...
				        <Table.HeaderCell style={{width: '7%'}}>Customer ID</Table.HeaderCell>
